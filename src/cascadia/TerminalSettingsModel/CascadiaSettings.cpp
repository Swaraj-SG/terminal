...
{
    auto fragment{ winrt::make_self<FragmentSettings>(_source) };
    fragment->_source = _source;

    std::vector<Model::Profile> modifiedProfiles;
    modifiedProfiles.reserve(_modifiedProfiles.Size());
    for (const auto& profile : _modifiedProfiles)
    {
        modifiedProfiles.emplace_back(*get_self<Profile>(profile)->CopySettings());
    }
    fragment->_modifiedProfiles = winrt::single_threaded_observable_vector(std::move(modifiedProfiles));

    std::vector<Model::Profile> newProfiles;
    newProfiles.reserve(_newProfiles.Size());
    for (const auto& profile : _newProfiles)
    {
        newProfiles.emplace_back(*get_self<Profile>(profile)->CopySettings());
    }
    fragment->_newProfiles = winrt::single_threaded_observable_vector(std::move(newProfiles));

    std::vector<Model::ColorScheme> colorSchemes;
    colorSchemes.reserve(_colorSchemes.Size());
    for (const auto& scheme : _colorSchemes)
    {
        colorSchemes.emplace_back(*get_self<ColorScheme>(scheme)->Copy());
    }
